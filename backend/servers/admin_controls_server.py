...
# def approved(user,required_role) :
#     user_role = user["role"]
#     if user_role in 
async def get_current_user(user_service : IUserService = Depends(get_user_service),token = Depends(oauth_scheme)) :
    try:
...
# @router.get("/all")
# async def get_all(user_service : IUserService = Depends(get_user_service),user = Depends(oauth_scheme)) -> UserCollection:
#     if approved()
#     users_service = await user_service.get_all()
#     return UserCollection(Users=users_service)
    
@router.get("/delete_database")
async def delete_all(user_service : IUserService = Depends(get_user_service),user = Depends(oauth_scheme)) -> str:
